...
class MNIST8M(data.Dataset):
    ...
    def download(self):
        ...
        training_set = (
            read_image_file(os.path.join(self.root, self.raw_folder, 'train-patterns')),
            read_label_file(os.path.join(self.root, self.raw_folder, 'train-labels'))
        )
        test_set = (
            read_image_file(os.path.join(self.root, self.raw_folder, 'test-patterns')),
            read_label_file(os.path.join(self.root, self.raw_folder, 'test-labels'))
        )
        with open(os.path.join(self.root, self.processed_folder, self.training_file), 'wb') as f:
            torch.save(training_set, f)
        with open(os.path.join(self.root, self.processed_folder, self.test_file), 'wb') as f:
...
def read_image_file(path):
    ...
    # with codecs.open(path, 'r', encoding='utf-8', errors='ignore') as f:
        ...
        # return torch.from_numpy(data).view(length, num_rows, num_cols)
